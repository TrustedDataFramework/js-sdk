...
export function encode(
  ...
  if (typeof o === 'string') return encodeString(o)
  if (typeof o === 'number') {
    assert(
      o >= 0 && Number.isInteger(o),
      `${o} is not a valid non-negative integer`
    )
    return encodeBytes(numberToByteArray(o))
  }
  if (typeof o === 'boolean') return o ? new Uint8Array([0x01]) : NULL_RLP
  if (o instanceof Uint8Array) return encodeBytes(o)
 ...
 * @param e
 */
export function decode(e: Uint8Array | ArrayBuffer): RLPElement | RLPElement[] {
      ...
      return (
        byteArrayToInt(
          copyOfRange(
            this.buf,
            1 + this.offset,
            1 + this.offset + prefix - OFFSET_LONG_ITEM
          )
        ) +
        1 +
        prefix -
    ...
    return (
      byteArrayToInt(
        copyOfRange(
          this.buf,
          1 + this.offset,
          this.offset + 1 + prefix - OFFSET_LONG_LIST
        )
      ) +
      1 +
      prefix -
...
export class RLPList {
  ...
  isNull(index: number): boolean {
    return (
      this.elements[index].byteLength == 1 && this.elements[index][0] == 0x80
    )
  }
