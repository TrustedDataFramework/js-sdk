...
export function padPrefix(arr: Uint8Array, prefix: number, size: number) {
  ...
  for (let i = 0; i < size - arr.length; i++) ret[i] = prefix
}
...
export function publicKey2Address(pk: Binary): string {
  ...
  const ec = getEC()
  if (ec === 'sm2') {
    if (k.length !== 33) throw new Error('invalid public key size: ' + k.length)
  }
  if (ec === 'ed25519') {
    if (k.length !== 32) throw new Error('invalid public key size: ' + k.length)
  }
  const buf = digest(k)
...
export function normalizeAddress(addr: Binary): string {
  let b = hex2bin(addr)
  // public key
  if (b.length === 33 || b.length === 32) return publicKey2Address(addr)
  if (b.length === 20) return bin2hex(b)
...
export function bin2hex(input: Binary | number[]): string {
  ...
  if (
    ...
    throw new Error(
      'input ' +
        input +
        ' is not ArrayBuffer Uint8Array or Buffer and other array-like '
    )
  if (!(input instanceof Uint8Array)) input = new Uint8Array(input)
  // input maybe Buffer or Uint8Array
  return Array.prototype.map
    .call(input, (x) => ('00' + x.toString(16)).slice(-2))
    .join('')
}
...
export function convert(
  o: string | Uint8Array | number | ArrayBuffer | boolean | bigi,
  type: ABI_DATA_TYPE
): Uint8Array | string | bigi | number {
  ...
  if (o instanceof Uint8Array) {
    ...
    switch (type) {
      ...
      case ABI_DATA_TYPE.address: {
        assert(
          o.length === 20,
          `the length of address is 20 while ${o.length} found`
        )
        return o
      }
  ...
  if (typeof o === 'string') {
    switch (type) {
      ...
      case ABI_DATA_TYPE.bool: {
        let l = o.toLowerCase()
        if ('true' === l || '1' === o) return ONE
        if ('false' === l || '0' === o) return ZERO
        throw new Error(
          `convert ${o} to bool failed, provide true, 1 or false, 0`
        )
      }
      case ABI_DATA_TYPE.bytes: {
        return hex2bin(o)
      ...
      case ABI_DATA_TYPE.address: {
        ...
        assert(
          a.length === 20,
          `the length of address is 20 while ${a.length} found`
        )
        return a
      }
    }
    ...
    switch (type) {
      ...
      case ABI_DATA_TYPE.u64: {
        ...
        if (type === ABI_DATA_TYPE.u64)
          assert(
            cmp(o, MAX_U64) <= 0 && cmp(o, ZERO) >= 0,
            `${o.toString(10)} overflows u64 ${MAX_U64.toString(10)}`
          )
        if (type === ABI_DATA_TYPE.u256)
          assert(
            cmp(o, MAX_U256) <= 0 && cmp(o, ZERO) >= 0,
            `${o.toString(10)} overflows max u256 ${MAX_U256.toString(10)}`
          )
        return o
      }
      case ABI_DATA_TYPE.string: {
      ...
      case ABI_DATA_TYPE.bool: {
        if (cmp(o, ZERO) === 0 || cmp(o, ONE) === 0) return o
        throw new Error(`convert ${o} to bool failed, provide 1 or 0`)
      }
      case ABI_DATA_TYPE.i64: {
        assert(
          o <= MAX_I64,
          `${o.toString(10)} overflows max i64 ${MAX_I64.toString(10)}`
        )
        assert(
          o >= MIN_I64,
          `${o.toString(10)} overflows min i64 ${MIN_I64.toString(10)}`
        )
        if (cmp(o, ZERO) >= 0) return o
        return i64ToLong(o)
      }
      case ABI_DATA_TYPE.f64: {
        // 大整数转 float 可能会丢失精度
        ...
        console.warn(
          `cast bigint ${o} to float may lose precision, you should always avoid it`
        )
...
export function decodeBE(x: ArrayBuffer | Uint8Array): bigi {
  if (x instanceof Uint8Array && x.length === 0) return ZERO
  if (x instanceof ArrayBuffer && x.byteLength === 0) return ZERO
...
export function encodeBE(x: number | bigi, bits?: number): Uint8Array {
  if (cmp(toBigN(x), ZERO) === 0) return new Uint8Array(0)
  let str = x.toString(16)
  str = str.length % 2 === 0 ? str : '0' + str
  let arr = hex2bin(str)
  if (bits === undefined) return arr
  let bytes = bits / 8
  if (Number.isInteger(bytes) && bytes < 0)
    throw new Error(`invalid bits number ${bits}, should be positive and `)
  if (arr.length > bytes) throw new Error('encode failed, overflow')
 ...
 */
export function toBigN(
  x: string | number | bigi | ArrayBuffer | Uint8Array
): bigi {
  ...
  if (typeof x === 'string') {
    if (typeof BigInt === 'function') return BigInt(x)
    return new BN(
      x.startsWith('0x') ? x.substring(2) : x,
      x.startsWith('0x') ? 16 : 10,
      'be'
    )
  }
...
export function doSign(sk: Binary, data: Binary): Uint8Array {
  ...
  if (ec === 'sm2') {
    return hex2bin(
      sm2.doSignature(data, bin2hex(sk), {
...
export function generatePrivateKey(): string {
  const ec = getEC()
  if (ec === 'sm2') return sm2.generateKeyPairHex().privateKey
...
export function createKeyStore(
  ...
  let key = hex2bin(sm3(concatBytes(salt, str2bin(password))))

  key = key.slice(0, 16)

  let cipherText = sm4.encrypt(_priv, key)
...
export function readKeyStore(ks: KeyStore, password: string): string {
  ...
  let k = hex2bin(sm3(concatBytes(salt, str2bin(password))))
  k = k.slice(0, 16)
...
export function createAuthServer(
  ...
  server.on('request', function (request, response) {
    ...
    if (whiteList && whiteList.length) {
      const exists = whiteList
        .map((x) => bin2hex(x) === otherPublicKey)
        .reduce((x, y) => x || y, false)
      if (!exists)
        throw new Error(
          `invalid public key, not in white list ${otherPublicKey}`
        )
    }
