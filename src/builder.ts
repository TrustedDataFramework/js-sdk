...
export class TransactionBuilder {
  ...
  buildDeploy(
    ...
    assert(
      contract.binary && contract.binary instanceof Uint8Array,
      'contract binary is not uint8 array'
    )
    ...
    let hasConstructor =
      contract.abi.filter((x) => x.name === 'init').length > 0
    let encoded = hasConstructor
      ? contract.abiEncode('init', parameters)
      : [[], [], []]

    const ret = this.buildCommon(
      constants.DEPLOY,
      amount,
      rlp.encode([binary, encoded, contract.abiToBinary()]),
      ''
    )
  ...
  buildContractCall(
    ...
    const ret = this.buildCommon(
      constants.CONTRACT_CALL,
      amount,
      rlp.encode([method, encoded]),
      addr
    )
    ret.__abi = contract.abi
