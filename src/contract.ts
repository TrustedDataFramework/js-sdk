...
class MemoryOutputStream implements OutputStream {
  ...
  write(chunk: string | Uint8Array): void {
    ...
    else this.buf = concatBytes(this.buf, chunk)
  }
}
...
export async function compileContract(
  ascPath?: string,
  src?: string,
  opts?: { debug?: boolean; optimize?: boolean }
): Promise<Uint8Array> {
  let abortPath = path.relative(
    process.cwd(),
    path.join(__dirname, '../lib/prelude/abort')
  )
  abortPath = abortPath.replace(/\\/g, '/')
  if (
    typeof ascPath === 'string' &&
    ascPath.trim() &&
    typeof src === 'string'
  ) {
    ...
    let cmd =
      ascPath + ' ' + src + ` -b --runtime none --use abort=${abortPath}` // 执行的命令
  ...
  if (typeof src !== 'string') throw new Error('invalid source file ' + src)
  const arr = [src, '-b', '--runtime', 'none', '--use', `abort=${abortPath}`]
  const stdout = new MemoryOutputStream()
  const stderr = new MemoryOutputStream()

  if (opts && opts.debug) arr.push('--debug')
  if (opts && opts.optimize) arr.push('--optimize')
...
export function normalizeParams(
  params?: AbiInput | AbiInput[] | Record<string, AbiInput>
): AbiInput[] | Record<string, AbiInput> {
...
export function abiDecode(
  outputs: TypeDef[],
  buf?: Uint8Array[]
): Readable[] | Record<string, Readable> {
  buf = buf || []
  ...
  if (arr.length != outputs.length)
    throw new Error(
      `abi decode failed , expect ${outputs.length} returns while ${arr.length} found`
    )
    ...
    switch (t) {
      ...
      case 'u64': {
        ...
        if (t === 'u64')
          assert(
            cmp(n, MAX_U64) <= 0,
            `${n.toString(10)} overflows max u64 ${MAX_U64.toString(10)}`
          )
        if (t === 'u256')
          assert(
            cmp(n, MAX_U256) <= 0,
            `${n.toString(10)} overflows max u256 ${MAX_U256.toString(10)}`
          )
        val = toSafeInt(n)
        break
      }
...
export class Contract {
  ...
  constructor(
    ...
    if (address) this.address = bin2hex(address)
    if (abi) this.abi = (abi || []).map(ABI.from)
    if (binary)
      this.binary =
  ...
  abiEncode(
    name: string,
    li?: AbiInput | AbiInput[] | Record<string, AbiInput>
  ): AbiEncoded {
    ...
    if (
      typeof li === 'string' ||
      typeof li === 'number' ||
      li instanceof ArrayBuffer ||
      li instanceof Uint8Array ||
      typeof li === 'boolean' ||
      typeof li === 'bigint'
    )
    ...
    if (Array.isArray(li)) {
      ...
      if (li.length != func.inputs.length)
        throw new Error(
          `abi encode failed for ${func.name}, expect ${func.inputs.length} parameters while ${li.length} found`
        )
  ...
  abiDecode(
    ...
    buf?: Uint8Array[],
    type?: ABI_TYPE
  ): Readable | Readable[] | Record<string, Readable> {
    type = type || 'function'
    buf = buf || []
    ...
    if (type === 'function') return ret && ret[0]
    return ret
  }
  ...
  getABI(name: string, type: ABI_TYPE): ABI {
    ...
    assert(
      funcs.length === 1,
      `exact exists one and only one abi ${name}, while found ${funcs.length}`
    )
    return funcs[0]
  }
}
    ...
    for (let p of str.split(',')) {
      ...
      if (event) {
        if (!l.startsWith('readonly'))
          throw new Error(
            `event constructor field ${l} should starts with readonly`
          )
        l = l.split(' ')[1]
      }
      const r = lr[1].trim()
      const o = new TypeDef(TYPES[r], l)
      if (!o.type) throw new Error(`invalid type: ${r}`)
      ret.push(o)
    }
    return ret
  }
