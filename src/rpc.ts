...
export class RPC {
  ...
  private __listen(
    contract: Contract,
    event: string,
    func: (r: Record<string, Readable>) => void
  ) {
    ...
    const fn = (r: EventResp | TransactionResp) => {
      ...
      const abiDecoded = <Record<string, Readable>>(
        contract.abiDecode(resp.name, <Uint8Array[]>resp.fields, 'event')
      )
      func(abiDecoded)
    }
  ...
  removeListener(id: number): void {
    ...
    if (key) {
      ...
      set && set.delete(id)
      if (set && set.size === 0) this.eventHandlers.delete(key)
    }
    if (h) {
      const set = this.txObservers.get(h)
      set && set.delete(id)
      if (set && set.size === 0) this.txObservers.delete(h)
    }
  }
  ...
  listenOnce(
    contract: Contract,
    event: string
  ): Promise<Record<string, Readable>> {
    const id = this.cid + 1
  ...
  private __observe(h: Binary, cb: (r: TransactionResp) => void) {
    ...
    if (!this.txObservers.has(hash)) this.txObservers.set(hash, new Set())
  ...
  viewContract(
    contract: Contract,
    method: string,
    parameters?: AbiInput | AbiInput[] | Record<string, AbiInput>
  ): Promise<Readable> {
      ...
      this.__observe(tx.getHash(), (r: TransactionResp) => {
        ...
        if (r.status === TX_STATUS.INCLUDED) {
          ...
          if (
            ...
            const decoded = new Contract('', tx.__abi).abiDecode(
              tx.getMethod(),
              <Uint8Array[]>r.result
            )
          ...
          if (r.events && r.events.length && tx.__abi) {
            const events = []
  ...
  private wsRPC(code, data: any): Promise<Resp> {
    ...
    this.tryConnect().then(() => {
      ...
      if (this.ws) this.ws.send(encoded)
    })
    return ret
  }
  ...
  sendAndObserve(
    ...
    if (Array.isArray(tx)) {
      const arr = []
      sub = this.wsRPC(
        WS_CODES.TRANSACTION_SUBSCRIBE,
        tx.map((t) => hex2bin(t.getHash()))
      )
      for (const t of tx) {
    ...
    return sub.then(() => this.sendTransaction(tx)).then(() => ret)
  }
  ...
  async getHeader(hashOrHeight: string | number): Promise<Header> {
    ...
    const url = `${protocol}${this.host}:${this.port}/rpc/header/${hashOrHeight}`
  ...
  mapBlock(data: any): Block {
    data.createdAt =
      typeof (<any>data.createdAt) === 'string' &&
      OFFSET_DATE.test(<any>data.createdAt)
        ? new Date(<any>data.createdAt)
        : new Date(<any>data.createdAt * 1000)
  ...
  async getBlock(hashOrHeight: string | number): Promise<Block> {
    const protocol = this.ssl ? 'https://' : 'http://'
    const url = `${protocol}${this.host}:${this.port}/rpc/block/${hashOrHeight}`
  ...
  getTransaction(hash: string): Promise<Transaction> {
    const protocol = this.ssl ? 'https://' : 'http://'
    const url = `${protocol}${this.host}:${this.port}/rpc/transaction/${hash}`
    return rpcGet(url).then((r: any) => {
   ...
   * @returns {Promise<Object>}
   */
  getAccount(pkOrAddress): Promise<Account> {
    let d = hex2bin(pkOrAddress)
    if (d.length != 20) d = hex2bin(publicKey2Address(d))
    return this.wsRPC(WS_CODES.ACCOUNT_QUERY, d).then((resp) => {
  ...
  getNonce(pkOrAddress) {
    return this.getAccount(pkOrAddress).then((a) => a.nonce)
  }
  ...
  getAuthPending(contractAddress: Binary) {
    return rpcPost(
      this.host,
      this.port,
      `/rpc/contract/${bin2hex(contractAddress)}`,
      {
        method: 'pending'
      },
      this.ssl
  ...
  getPoSNodeInfos() {
    return rpcPost(
      this.host,
      this.port,
      `/rpc/contract/${constants.POS_CONTRACT_ADDR}`,
      {
        method: 'nodeInfos'
      },
      this.ssl
...
function rpcGet(url: string): Promise<Object> {
...
function rpcPost(
  ...
  if (typeof window !== 'object') {
    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.open('POST', `${protocol}${host}:${port}${path}`)
