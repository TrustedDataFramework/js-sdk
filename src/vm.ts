...
export class WasmInterface {
  ...
  put(offset: number | bigint, data: ArrayBuffer): void {
    new Uint8Array(this.ins.exports.memory.buffer).set(
      new Uint8Array(data),
      Number(offset)
    )
  }
  ...
  malloc(
    x: string | bigint | ArrayBuffer | Uint8Array | number | boolean,
    type: ABI_DATA_TYPE
  ): number | bigint {
    let bin: ArrayBuffer
    ...
    this.put(p0, bin)
    const ptr = this.ins.exports.__change_t(
      BigInt(type),
      p0,
...
export class VirtualMachine {
  ...
  dump(): ArrayBuffer {
    ...
    ][] = storageArray.map((x) => [x[0], this.map2Array(x[1])])
    const arr = [
      this.height,
      this.parentHash,
      this.hash,
      this.map2Array(this.contractCode),
      this.map2Array(this.nonceMap),
      this.map2Array(this.balanceMap),
      this.now,
      enc
    ]
  ...
  static fromDump(dumped: ArrayBuffer | Uint8Array): VirtualMachine {
    ...
    ;(decoded[7] as any[]).forEach(
      (arr: [Uint8Array, [Uint8Array, Uint8Array][]]) => {
        const k = bin2hex(arr[0])
        if (!vm.storage.has(k)) vm.storage.set(k, new Map())
  ...
  subBalance(addr: Binary, amount?: Digital) {
    ...
    if (balance < a) throw new Error(`the balance of ${hex} is not enough`)
    balance = balance - a
    this.balanceMap.set(hex, balance)
  }
  ...
  private async createInstance(file: string, env: any): Promise<VMInstance> {
    ...
    const fs = require('fs')
    return <VMInstance>(
      (await WebAssembly.instantiate(fs.readFileSync(file), { env: env }))
        .instance
    )
  }
  ...
  async callInternal(
    ...
    if (a.outputs && a.outputs.length)
      r.result = this.extractRet(
        instance,
        ret,
        ABI_DATA_TYPE[a.outputs[0].type]
      )
    return r
  }
  ...
  async fetchABI(wasmFile: string): Promise<ABI[]> {
    ...
    if (this.abiCache.has(f)) return this.abiCache.get(f)
