...
export class Transaction implements Encoder {
  ...
  constructor(
    ...
    if (__abi) this.__abi = __abi
    if (__inputs) this.__inputs = __inputs
  }
  ...
  __setInputs(__inputs: AbiInput[] | Record<string, AbiInput>): void {
    const cnv: (i: AbiInput) => Readable = (x: AbiInput) => {
      if (x instanceof ArrayBuffer || x instanceof Uint8Array) return bin2hex(x)
      if (x instanceof BN) return x.toString(10)
      return x
    }
  ...
  getMethod() {
    ...
    return t === constants.DEPLOY
      ? 'init'
      : bin2str(<Uint8Array>rlp.decode(hex2bin(this.payload))[0])
  }
  ...
  isBuiltInCall() {
    return (
      this.isDeployOrCall() &&
      (this.to === constants.PEER_AUTHENTICATION_ADDR ||
        this.to === constants.POA_AUTHENTICATION_ADDR ||
